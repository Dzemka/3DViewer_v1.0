...
class Gif {
 public:
  ...
  int GifPartition(uint8_t *image, const int left, const int right,
  ...
  // Writes the EOF code, closes the file handle, and frees temp memory used by
  // a GIF. Many if not most viewers will still display a GIF properly if the
  // EOF code is missing, but it's still a good idea to write it out.
