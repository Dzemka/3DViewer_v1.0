...
static char **fill_td_array(char **dst, char *src, char divider,
                            int count_str) {
  int s_index;
  int c_index;
  int len_string;
  ...
  while (s_index < count_str) {
    c_index = 0;
    while (src[src_index] == divider) src_index++;
    len_string = ft_len_string(src, divider, src_index);
    dst[s_index] = (char *)(malloc(sizeof(char) * (len_string + 1)));
    if (dst[s_index] == NULL) return (clear_leaks(dst, s_index));
    while (c_index < len_string) dst[s_index][c_index++] = src[src_index++];
